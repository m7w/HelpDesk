...
public class TicketControllerTest extends AbstractControllerTest {
    ...
    private static final Ticket TICKET1 = new Ticket(TICKET1_ID, "Ticket1", "Ticket1 description",
        LocalDate.of(2022, 7, 10), LocalDate.of(2022, 8, 10), ENGINEER, EMPLOYEE, State.NEW,
        CATEGORY, Urgency.AVERAGE, MANAGER);
    private static final Ticket TICKET2 = new Ticket(TICKET2_ID, "Ticket2", "Ticket2 description",
        LocalDate.of(2022, 7, 11), LocalDate.of(2022, 8, 11), ENGINEER, EMPLOYEE, State.APPROVED,
        CATEGORY, Urgency.AVERAGE, MANAGER);
    ...
    public void testGetAllTickets_WithDefaults() throws Exception {
        ...
        when(ticketService.findAllByUser(any(QueryMetadata.class)))
                .thenReturn(new Page<TicketSmallDto>(2L, List.of(ticketSmallDto1, ticketSmallDto2)));
        ...
        mockMvc.perform(get("/api/tickets"))
            ...
            .andExpect(jsonPath("$.entities[0].actions[0].label", is(Action.APPROVE.getLabel())))
            .andExpect(jsonPath("$.entities[0].actions[1].key", is(Action.DECLICE.getKey())))
            .andExpect(jsonPath("$.entities[0].actions[1].label", is(Action.DECLICE.getLabel())))
            ...
            .andExpect(jsonPath("$.entities[1].actions[0].key", is(Action.ASSIGN_TO_ME.getKey())))
            .andExpect(jsonPath("$.entities[1].actions[0].label", is(Action.ASSIGN_TO_ME.getLabel())))
            .andExpect(jsonPath("$.entities[1].actions[1].key", is(Action.CANCEL.getKey())))
            .andExpect(jsonPath("$.entities[1].actions[1].label", is(Action.CANCEL.getLabel())));
    ...
    public void testGetTicketById_HasAccess() throws Exception {
        when(accessChecker.hasAccess(TICKET1_ID))
            .thenReturn(true);
        when(ticketService.findById(TICKET1_ID))
